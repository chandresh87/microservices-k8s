...
public class OrganizationServiceData {
	...
	public OrganizationDTO getOrganizationData(int organizationId,String auth) {
		...
		// randomlyRunLong(); // Testing circuit breaker
			...
			organizationDTO = organizationRemoteData.getRemoteOrgData(organizationId,auth);
			return organizationDTO;
	}


	// FallBack method used by the Hytrix
	private OrganizationDTO buildFallBackOrganization(int OrganizationId,String auth,Throwable throwable) {
		logger.info("calling fallback method to get the organization data for id {}", OrganizationId);
		logger.info("Exception to get the organization data for id {}", throwable);
